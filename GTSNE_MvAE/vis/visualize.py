...
from GTSNE_MvAE.utils.crack_number import crack
import numpy as np
import logging
import pandas as pd
...
from GTSNE_MvAE.vis.plots import plot_gnd_hat
...
def plot_scatters(H, args: dict, size=10, palette='viridis', markers='o'):
...
def heatmap(X, title=None, cmap='gray'):
    sns.heatmap(X, cmap=cmap, cbar=None, xticklabels=[], yticklabels=[])
    if title:
        plt.title(title)
